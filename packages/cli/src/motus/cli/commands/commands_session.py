...
def summary_command(session_id: Optional[str] = None) -> None:
    ...
    summary = (
        "## MC Session Memory\n\n> Auto-generated by Motus Command. Add this to "
    ...
    for condition, message in (
        (
            ...
            "- ⚠️ Multiple high-risk operations were performed. Review changes carefully.",
        ),
        (
            len(stats.files_modified) > 10,
            "- 📁 Many files were modified. Consider committing before continuing.",
        ),
        (
            stats.agent_count > 3,
            "- 🤖 Multiple subagents were spawned. Check for coordination issues.",
        ),
        (stats.tool_count > 100, "- ⚡ High tool usage. The session was very active."),
    ):
        if condition:
            summary += f"{message}\n"
    summary += (
